...
// Yetenekleri kategorize edelim. Bu veriyi normalde ayrı bir dosyadan da çekebilirdik.
const SKILLS = [
  ...
  return (
    ...
    <section className="min-h-screen pt-24 pb-16 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20">
        ...
        <div className="relative h-[400px] w-full rounded-2xl overflow-hidden bg-gray-200 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-xl flex items-center justify-center group">
             {/* Buraya kendi fotoğrafını koyabilirsin. Şimdilik soyut bir alan. */}
      ...
      {/* Alt Kısım: Yetenekler (Tech Stack) */}
      <div className="space-y-8">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-800 pb-4">
          Teknik Yetkinlikler
        </h2>
        
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {SKILLS.map((skillGroup, index) => (
            <div key={index} className="bg-white dark:bg-gray-900 p-6 rounded-xl border border-gray-200 dark:border-gray-800 hover:border-blue-500 transition-colors shadow-sm">
              <h3 className="text-xl font-semibold text-blue-600 mb-4">
                {skillGroup.category}
              </h3>
              <div className="flex flex-wrap gap-2">
                {skillGroup.items.map((item, i) => (
                  <span key={i} className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full text-sm font-medium">
                    {item}
                  </span>
                ))}
              </div>
